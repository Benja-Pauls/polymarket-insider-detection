...
possess non-public information about the event being wagered on. We construct
an on-chain dataset of \textit{N} resolved Polymarket prediction markets
(totalling \textit{M} trades across \textit{W} distinct wallets) using
Polymarket's public subgraphs on Goldsky. For each market we engineer
features capturing the joint distribution of volume, directional flow, price
acceleration, and wallet concentration over multiple horizons before
resolution. Using a layered labeling scheme that combines heuristic weak
positives with publicly-alleged ``whale'' trades from news archives, we
train and evaluate logistic, random-forest, and gradient-boosted classifiers.
Our best model achieves \textit{XX} ROC-AUC and \textit{YY} precision@20 on
a held-out test split of markets, substantially above a random baseline.
Feature-importance analysis implicates the \textit{last-24-hour volume
spike ratio}, \textit{top-1 wallet share}, and \textit{price-acceleration
z-score} as the three most discriminative signals. We discuss limitations
(anonymous wallet identities, survival bias, and the challenge of false
positives from scheduled-information events such as FOMC meetings) and
release the entire pipeline --- data collection, features, models, and
figures --- as a reproducible public artifact.
\end{abstract}
...
meets or exceeds \textit{\$VMIN}. This yields a catalog of
$N = $ \textit{NMARKETS} markets resolved between \textit{DATE\_MIN} and
\textit{DATE\_MAX}.
...
\begin{figure}[t]
  ...
  \caption{ROC and precision-recall curves on the held-out test split.}
  \label{fig:roc_pr}
\end{figure}
